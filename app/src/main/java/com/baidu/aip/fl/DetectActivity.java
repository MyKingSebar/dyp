...
import v1.cn.unionc_pad.utils.BitmapFileSetting;
    ...
    private void showFrame(FaceFilter.TrackedModel model) {
        ...
        if (model != null) {
            ...
            if (mFrameIndex >= 10) {
                ...
                String mHeadCachePath = Environment.getExternalStorageDirectory().getPath() + File.separator +  "yibashiPad" ;
                File parentPath = new File(mHeadCachePath);
                if (!parentPath.exists()) {
                    parentPath.mkdirs();
                }
                File mHeadCacheFile = new File(parentPath, "face.png");
                ...
                if (face != null) {
                    File f= BitmapFileSetting.saveBitmapFile(face, Environment.getExternalStorageDirectory()+ "/yibashiPad/face.png");
                    if(f!=null){
                        getBaidu(f);
                    }
...
//                    mList.add(face);
    ...
    private void displayBriefMemory()
    {
        ...
        Log.i("linshi","系统剩余内存:"+(info.availMem >> 10)+"k");
        Log.i("linshi","系统是否处于低内存运行："+info.lowMemory);
        Log.i("linshi","当系统剩余内存低于"+info.threshold+"时就看成低内存运行");
    }
